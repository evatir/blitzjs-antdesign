...
var ModelGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.write = async function write() {
    try {
      if (!this.fs.exists(path__default.resolve("db/schema.prisma"))) {
      ...
      if (!this.options.dryRun) {
        ...
        var schema = this.fs.read(path__default.resolve("db/schema.prisma"));

        if (schema.indexOf("model " + modelDefinition.name) === -1) {
          //model does not exist in schema - just add it
          this.fs.append(path__default.resolve("db/schema.prisma"), "\n" + modelDefinition.toString() + "\n");
        } else {
          ...
          if (model) {
            ...
            this.fs.write(path__default.resolve("db/schema.prisma"), schema.replace(model, newModel));
      ...
      log.newline();
      log.success("Model for '" + this.options.modelName + "'" + (this.options.dryRun ? "" : " " + updatedOrCreated + " in schema.prisma") + ":\n");
