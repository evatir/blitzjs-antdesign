...
var Field = /*#__PURE__*/function () {
  ...
  Field.parse = function parse(input) {
  ...
  _proto.getRelation = function getRelation() {
    ...
    var separator = this.relationToFields && this.relationToFields.length > 0 && this.relationFromFields && this.relationFromFields.length ? ", " : "";
    var fromFields = this.relationFromFields && this.relationFromFields.length > 0 ? "fields: [" + this.relationFromFields.toString() + "]" : "";
    var toFields = this.relationToFields && this.relationToFields.length > 0 ? "references: [" + this.relationToFields.toString() + "]" : "";
    return "@relation(" + fromFields + separator + toFields + ")";
  };

  _proto.getTypeModifiers = function getTypeModifiers() {
    return "" + (this.isRequired ? "" : "?") + (this.isList ? "[]" : "");
  };

  _proto.getAttributes = function getAttributes() {
    var possibleAttributes = [this.getDefault(), this.getId(), this.getIsUnique(), this.getIsUpdatedAt(), this.getRelation()]; // filter out any attributes that return ''

    var attrs = possibleAttributes.filter(function (attr) {
      return attr;
    });
...
function stringifyFieldsForPrinting(fields) {
  var maxNameLength = 1;
  var maxTypeLength = 1;
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(fields), _step; !(_step = _iterator()).done;) {
    ...
    var _field$toString$split2 = field.toString().split(/[\s]+/),
        name = _field$toString$split2[0],
        type = _field$toString$split2[1];
  ...
  return fields.map(function (field) {
    var _field$toString$split = field.toString().split(/[\s]+/),
        name = _field$toString$split[0],
        type = _field$toString$split[1],
        rest = _field$toString$split.slice(2);

    var attrs = rest.join(" ");
    var namePad = maxNameLength - name.length;
    var typePad = maxTypeLength - type.length;
...
var Model = /*#__PURE__*/function () {
  ...
  _proto.getFields = function getFields() {
    return stringifyFieldsForPrinting([this.getIdField(), this.getCreatedAtField(), this.getUpdatedAtField()].concat(this.fields)).map(function (field) {
      return "\n  " + field;
    }).join("");
  };

  _proto.getNewFields = function getNewFields() {
    return stringifyFieldsForPrinting([].concat(this.fields)).map(function (field) {
      return "  " + field + "\n";
    }).join("");
  };
...
function matchBetween(test, start, end) {
  var re = new RegExp(start + "([\\s\\S]*?)" + end, "g");
...
var ModelGenerator = /*#__PURE__*/function (_Generator) {
  ...
  _proto.write = async function write() {
    try {
      if (!this.fs.exists(path__namespace['default'].resolve("db/schema.prisma"))) {
        throw new Error("Prisma schema file was not found");
      }

      var updatedOrCreated = "created";
      var extraArgs = this.options.extraArgs.length === 1 && this.options.extraArgs[0].includes(" ") ? this.options.extraArgs[0].split(" ") : this.options.extraArgs;
      var modelDefinition = new Model(this.options.modelName, extraArgs.flatMap(function (def) {
        return Field.parse(def);
      }));

      if (!this.options.dryRun) {
        // wrap in newlines to put a space below the previously generated model and
        // to preserve the EOF newline
        var schema = this.fs.read(path__namespace['default'].resolve("db/schema.prisma"));

        if (schema.indexOf("model " + modelDefinition.name) === -1) {
          //model does not exist in schema - just add it
          this.fs.append(path__namespace['default'].resolve("db/schema.prisma"), "\n" + modelDefinition.toString() + "\n");
        } else {
          var model = matchBetween(schema, "model " + modelDefinition.name, "}");

          if (model) {
            //filter out all fields that are already defined
            modelDefinition.fields = modelDefinition.fields.filter(function (field) {
              return model.indexOf(field.name) === -1;
            }); //add new fields to the selected model

            var newModel = model.replace("}", modelDefinition.getNewFields() + "}"); //replace all content with the newly added fields for the already existing model

            this.fs.write(path__namespace['default'].resolve("db/schema.prisma"), schema.replace(model, newModel));
      ...
      display.log.newline();
      display.log.success("Model for '" + this.options.modelName + "'" + (this.options.dryRun ? "" : " " + updatedOrCreated + " in schema.prisma") + ":\n");
